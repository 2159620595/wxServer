...
function buildTemplateData(title, desp, remark) {
  ...
  const data = {
    ...
    [fieldBody]: { value: clipField(sanitizeForWechatMp(desp) || "—", 200) },
  };

  if (fieldRemark) {
    const remarkText = sanitizeForWechatMp(remark) || time;
    data[fieldRemark] = { value: clipField(remarkText, 200) };
  }
  return data;
}

async function pushWechatMp(title, desp, remark = "") {
  ...
  const resp = await fetch(`${TEMPLATE_URL}?access_token=${token}`, {
    ...
    body: JSON.stringify({
      ...
      data: buildTemplateData(title, desp, remark),
    }),
  });
